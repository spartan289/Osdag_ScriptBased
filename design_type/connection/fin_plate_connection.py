...
class FinPlateConnection(ShearConnection):
    ...
    def capacities(self, status):
        ...
        t17 = (KEY_OUT_PLATE_SHEAR, KEY_OUT_DISP_PLATE_SHEAR, TYPE_TEXTBOX,
               round(self.plate.shear_yielding_capacity / 1000, 2) if status else '')
        capacities.append(t17)

        t18 = (KEY_OUT_PLATE_RUPTURE, KEY_OUT_DISP_PLATE_RUPTURE, TYPE_TEXTBOX,
               round(self.plate.shear_rupture_capacity / 1000, 2) if status else '')
        capacities.append(t18)

        t17 = (KEY_OUT_PLATE_BLK_SHEAR, KEY_OUT_DISP_PLATE_BLK_SHEAR, TYPE_TEXTBOX,
               round(self.plate.block_shear_capacity_shear / 1000, 2) if status else '')
        capacities.append(t17)
    ...
    def section_capacities(self, status):
        ...
        t17 = (KEY_SHEAR_YIELDCAPACITY, KEY_OUT_DISP_PLATE_SHEAR, TYPE_TEXTBOX,
               round(self.supported_section.shear_yielding_capacity / 1000, 2) if status else '')
        capacities.append(t17)

        t18 = (KEY_SHEAR_RUPTURECAPACITY, KEY_OUT_DISP_PLATE_RUPTURE, TYPE_TEXTBOX,
               round(self.supported_section.shear_rupture_capacity / 1000, 2) if status else '')
        capacities.append(t18)
        ...
        t20 = (KEY_MEMBER_MOM_CAPACITY, KEY_OUT_DISP_PLATE_MOM_CAPACITY, TYPE_TEXTBOX,
               round(self.supported_section.moment_capacity / 1000000, 2) if status else '')
        capacities.append(t20)
    ...
    def output_values(self, flag):
        ...
        t3 = (
            KEY_OUT_GRD_PROVIDED, KEY_OUT_DISP_GRD_PROVIDED, TYPE_TEXTBOX,
        ...
        t8 = (
            KEY_OUT_BOLTS_ONE_LINE, KEY_OUT_DISP_BOLTS_ONE_LINE, TYPE_TEXTBOX,
            self.plate.bolts_one_line if flag else '',
            True)
        out_list.append(t8)
        ...
        t14 = (
            KEY_OUT_PLATETHK, KEY_OUT_DISP_PLATETHK, TYPE_TEXTBOX, int(self.plate.thickness_provided) if flag else '',
            True)
        out_list.append(t14)

        t15 = (
            KEY_OUT_PLATE_HEIGHT, KEY_OUT_DISP_PLATE_HEIGHT, TYPE_TEXTBOX, float(self.plate.height) if flag else '',
            True)
        out_list.append(t15)

        t16 = (
            KEY_OUT_PLATE_LENGTH, KEY_OUT_DISP_PLATE_LENGTH, TYPE_TEXTBOX, float(self.plate.length) if flag else '',
            True)
        out_list.append(t16)
        ...
        t15 = (
            KEY_OUT_WELD_STRENGTH, KEY_OUT_DISP_WELD_STRENGTH, TYPE_TEXTBOX,
            round(self.weld.strength, 2) if flag else '',
            True)
        out_list.append(t15)

        t16 = (
            KEY_OUT_WELD_STRESS, KEY_OUT_DISP_WELD_STRESS, TYPE_TEXTBOX, round(self.weld.stress, 2) if flag else '',
            True)
        out_list.append(t16)
    ...
    def select_bolt_dia(self):
        ...
        for self.bolt.bolt_diameter_provided in reversed(self.bolt.bolt_diameter):
            ...
            self.bolt.calculate_bolt_capacity(bolt_diameter_provided=self.bolt.bolt_diameter_provided,
                                              ...
                                              conn_plates_t_fu_fy=self.bolt_conn_plates_t_fu_fy,
                                              n_planes=1)
            print("getting web plate details for dia:", self.bolt.bolt_diameter_provided, self.bolt.bolt_grade_provided,
    ...
    def get_bolt_grade(self):
        ...
        for self.bolt.bolt_grade_provided in reversed(self.bolt.bolt_grade):
            ...
            print("for grade :", self.bolt.bolt_grade_provided, "capacity is:", self.bolt.bolt_capacity, "force is:",
                  self.plate.bolt_force)
    ...
    def get_fin_plate_details(self):
        ...
        for self.plate.thickness_provided in self.thickness_possible:
            ...
            [available_welds, self.weld_size_min, self.weld_size_max] = self.get_available_welds(self,
                                                                                                 self.weld_connecting_plates)
            if available_welds:
                    ...
                    if self.supported_section.moment_capacity > self.plate.moment_demand and self.plate.height + 10 <= self.max_plate_height:
                        ...
                        if self.plate.block_shear_capacity_axial > self.load.axial_force * 1000 or \
                            ...
                            self.plate.height = (self.plate.gauge_provided) * (
                                    self.plate.bolts_one_line - 1) + self.plate.edge_dist_provided
                            self.plate.get_web_plate_details(self.bolt.bolt_diameter_provided, self.plate.height,
                                                             self.plate.height,
                                                             self.bolt.bolt_capacity, self.plate.edge_dist_provided,
                                                             self.plate.gauge_provided,
                                                             self.plate.gauge_provided, self.plate.edge_dist_provided,
                                                             self.load.shear_force * 1000, self.load.axial_force * 1000,
                                                             0, self.plate.gap, True, 2,
                                                             self.plate.bolts_one_line, self.plate.bolt_line, None)
        ...
        if self.load.shear_force * 1000 > self.plate.shear_capacity:
            self.design_status = False
            logger.error(
                ": The shear capacity of the plate is less than the applied shear force, %2.2f kN [Ref. Cl.6.4.1, IS 800:2007]."
                % self.load.shear_force)
            logger.warning(
                ":The shear capacity of the plate is {} kN.".format(round(self.plate.shear_capacity / 1000, 2)))
            logger.info(": Increase the plate thickness or material grade.")

        if self.load.axial_force * 1000 > self.plate.tension_capacity:
            self.design_status = False
            logger.error(
                ":The tensile capacity of the plate is less than the applied axial force, %2.2f kN [Ref. Cl.6.4.1, IS 800:2007]."
                % self.load.axial_force)
            logger.warning(
                ":The tensile capacity of the plate is {} kN.".format(round(self.plate.tension_capacity / 1000, 2)))
            logger.info(": Increase the plate thickness or material grade.")

        if self.plate.moment_capacity < self.plate.moment_demand:
            self.design_status = False
            logger.error(
                ": The plate moment capacity is less than the moment demand, {} kNm [Ref. Cl.8.2.1.2, IS 800:2007]."
                .format(round(self.plate.moment_demand / 1000000, 2)))
            # print(self.plate.moment_capacity / 1000000)
            logger.warning(
                ": The moment capacity of the plate is {} kNm.".format(round(self.plate.moment_capacity / 1000000, 2)))
            logger.info(": Increase the plate thickness or material grade.")
        ...
        if self.load.shear_force * 1000 > self.supported_section.shear_capacity:
            self.design_status = False
            logger.error(
                ": The shear capacity of the beam is less than the applied shear force, %2.2f kN [Ref. Cl.6.4.1, IS 800:2007]."
                % self.load.shear_force)
            logger.warning(": The shear capacity of the beam is {} kN.".format(
                round(self.supported_section.shear_capacity / 1000, 2)))
            logger.info(
                ": Choose a beam of higher size or provide a larger bolt diameter (if available) to increase the rupture/block shear "
                "capacity.")

        if self.load.axial_force * 1000 > self.supported_section.tension_capacity:
            self.design_status = False
            logger.error(
                ": The tensile capacity of the beam is less than the applied axial force, %2.2f kN [Ref. Cl. 6.4.1, IS 800:2007]."
                % self.load.axial_force)
        ...
        if self.supported_section.moment_capacity < self.plate.moment_demand:
            self.design_status = False
            logger.error(
                ": The moment capacity of the beam is less than the moment demand, {} kNm [Ref. Cl. 8.2.1.2, IS 800:2007]."
                .format(round(self.plate.moment_demand / 1000000, 2)))
        ...
        if self.weld.strength < self.weld.stress:
            ...
            logger.warning(": The weld stress is {} N/mm and the weld strength is {} N/mm.".format(self.weld.stress,
                                                                                                   self.weld.strength))
            ...
            if self.weld.size in (3, 4):
                logger.info(
                    ": The minimum recommended weld throat thickness suggested by IS 800:2007 is 3 mm, as per " +
    ...
    def section_shear_checks(self):
        ...
        self.supported_section.block_shear_capacity_shear = IS800_2007.cl_6_4_1_block_shear_strength(A_vg, A_vn, A_tg,
                                                                                                     A_tn,
        ...
        if self.supported_section.shear_capacity < self.load.shear_force * 1000:
            ...
            logger.warning(
                'The shear capacity of the section is guiding plate height, current height is {} mm.'.format(
                    self.plate.height))
        else:
            self.supported_section.design_status = True

        self.supported_section.tension_rupture_capacity = IS800_2007.cl_6_3_1_tension_rupture_strength(A_vn,
                                                                                                       self.supported_section.fu)
        ...
        if self.supported_section.tension_capacity < self.load.axial_force * 1000:
            self.supported_section.design_status = False
            logger.warning(
                'The tension capacity of the section is guiding plate height, current height is {} mm.'.format(
                    self.plate.height))
        else:
            self.supported_section.design_status = True

        self.supported_section.moment_capacity = IS800_2007.cl_8_2_1_2_design_moment_strength \
            (self.supported_section.elast_sec_mod_z, self.supported_section.plast_sec_mod_z, self.supported_section.fy,
             'plastic')
        ...
        if self.supported_section.IR > 1 or self.supported_section.moment_capacity < self.plate.moment_demand or \
            ...
            logger.warning(
                'Axial - Moment interaction ratio of section is guiding plate height, current height is {} mm.'.format(
                    self.plate.height))
            self.supported_section.design_status = False
        else:
            self.supported_section.design_status = True
    ...
    def plate_shear_checks(self):
        ...
        self.plate.block_shear_capacity_shear = IS800_2007.cl_6_4_1_block_shear_strength(plate_A_vg, plate_A_vn,
                                                                                         plate_A_tg, plate_A_tn,
                                                                                         self.plate.fu,
                                                                                         self.plate.fy)
        ...
        if self.plate.tension_capacity < self.load.axial_force * 1000:
            self.plate.design_status_capacity = False
            logger.warning(
                'The tension capacity of the plate is guiding the plate height, current height is {} mm.'.format(
                    self.plate.height))
        else:
            self.plate.design_status_capacity = True
        ...
        if self.plate.moment_capacity < self.plate.moment_demand:
            logger.warning(
                'The moment capacity of the plate is guiding plate height, current height is {} mm.'.format(
                    self.plate.height))
            self.plate.design_status_capacity = False
        else:
            self.plate.design_status_capacity = True
        ...
        if self.plate.IR > 1 or self.plate.shear_capacity < self.load.shear_force * 1000 or self.plate.moment_capacity < self.plate.moment_demand:
            logger.warning(
                'Moment-Axial interaction ratio of plate is guiding plate height, current height is {} mm.'.format(
                    self.plate.height))
            self.plate.design_status_capacity = False
        else:
            self.plate.design_status_capacity = True
    ...
    def get_available_welds(self, connecting_members=[]):
        ...
        if weld_size_max == weld_size_min:
            logger.info(
                "The minimum weld size is greater than or equal to the thickness of the thinner connecting plate [Ref. Table 21, "
                "IS800:2007].")
    ...
    def save_design(self, popup_summary):
        ...
        if not self.thickness_possible and self.supported_section.design_status_initial is True:
            ...
            t1 = (DISP_MIN_PITCH, cl_10_2_2_min_spacing(self.bolt.bolt_diameter_provided),
                  self.plate.gauge_provided,
                  get_pass_fail(self.bolt.min_pitch, self.plate.gauge_provided, relation='leq'))
            self.report_check.append(t1)
            if self.plate.design_status is True:
                ...
                t2 = (DISP_MIN_GAUGE, cl_10_2_2_min_spacing(self.bolt.bolt_diameter_provided),
                      self.plate.pitch_provided,
                      get_pass_fail(self.bolt.min_gauge, self.plate.pitch_provided, relation="leq"))
            ...
            t3 = (DISP_MIN_END, cl_10_2_4_2_min_edge_end_dist(d_0=self.bolt.dia_hole,
                  ...
                  get_pass_fail(self.bolt.min_end_dist, self.plate.edge_dist_provided, relation='leq'))
            ...
            if self.plate.design_status is True:
                t4 = (DISP_MAX_END,
                      cl_10_2_4_3_max_edge_end_dist(self.bolt_conn_plates_t_fu_fy, self.bolt.corrosive_influences,
                                                    parameter='end_dist'),
                      self.plate.edge_dist_provided,
                      get_pass_fail(self.bolt.max_end_dist, self.plate.edge_dist_provided, relation='geq'))
                self.report_check.append(t4)
                t3 = (DISP_MIN_EDGE, cl_10_2_4_2_min_edge_end_dist(d_0=self.bolt.dia_hole,
                                                                   edge_type=self.bolt.edge_type,
                                                                   parameter='edge_dist'),
                      self.plate.end_dist_provided,
                      get_pass_fail(self.bolt.min_edge_dist, self.plate.end_dist_provided, relation='leq'))
                self.report_check.append(t3)
                t4 = (DISP_MAX_EDGE,
                      cl_10_2_4_3_max_edge_end_dist(self.bolt_conn_plates_t_fu_fy, self.bolt.corrosive_influences,
                                                    parameter='edge_dist'),
                      self.plate.end_dist_provided,
                      get_pass_fail(self.bolt.max_edge_dist, self.plate.end_dist_provided, relation="geq"))
                self.report_check.append(t4)
            if self.plate.design_status is False:
                ...
                if self.bolt.bolt_type == TYP_BEARING:
                    ...
                    t1 = (KEY_OUT_DISP_BOLT_SHEAR, '',
                          cl_10_3_3_bolt_shear_capacity(self.bolt.bolt_fu, 1, self.bolt.bolt_net_area,
                                                        self.bolt.gamma_mb, bolt_shear_capacity_kn), '')
                    self.report_check.append(t1)
                    t8 = (KEY_DISP_KB, " ",
                          cl_10_3_4_calculate_kb(self.plate.edge_dist_provided, self.plate.gauge_provided,
                                                 self.bolt.dia_hole,
                                                 self.bolt.bolt_fu, self.bolt.fu_considered), '')
                    self.report_check.append(t8)
                    t2 = (KEY_OUT_DISP_BOLT_BEARING, '',
                          cl_10_3_4_bolt_bearing_capacity(self.bolt.kb, self.bolt.bolt_diameter_provided,
                ...
                t10 = (KEY_OUT_LONG_JOINT, cl_10_3_3_1_long_joint_bolted_req(),
                       cl_10_3_3_1_long_joint_bolted_prov(self.plate.bolt_line, self.plate.bolts_one_line,
                                                          ...
                                                          self.bolt.bolt_diameter_provided, bolt_capacity_kn,
                ...
                t1 = (DISP_MIN_PLATE_HEIGHT,
                      min_plate_ht_req(self.supported_section.depth, self.supported_section.root_radius,
                ...
                for a in [self.plate, self.supported_section]:
                    ...
                    t1 = (KEY_DISP_SHEAR_YLD, '', cl_8_4_shear_yielding_capacity_member(h, t, a.fy, gamma_m0,
                                                                                        round(
                                                                                            a.shear_yielding_capacity / 1000,
                                                                                            2)), '')
                    self.report_check.append(t1)
                    ...
                    t1 = (KEY_DISP_ALLOW_SHEAR, display_prov(self.load.shear_force, "V"),
                          ...
                          get_pass_fail(self.load.shear_force, round(a.low_shear_capacity / 1000, 2),
                                        relation="lesser"))
                    self.report_check.append(t1)
                    ...
                    t1 = (KEY_DISP_PLATE_BLK_SHEAR_SHEAR, '',
                          cl_6_4_blockshear_capacity_member(Tdb=round(a.block_shear_capacity_shear / 1000, 2),
                                                            stress='shear'), '')
                    self.report_check.append(t1)
                    ...
                    t1 = (KEY_DISP_TENSION_YIELDCAPACITY, '',
                          cl_6_2_tension_yield_capacity_member(h, t, a.fy, gamma_m0,
                    ...
                    t1 = (KEY_DISP_PLATE_BLK_SHEAR_TENSION, '',
                          cl_6_4_blockshear_capacity_member(Tdb=round(a.block_shear_capacity_axial / 1000, 2),
                                                            stress='axial'), '')
                    self.report_check.append(t1)
                    ...
                    t1 = (KEY_DISP_IR, required_IR_or_utilisation_ratio(IR=1),
                          cl_9_3_combined_moment_axial_IR_section(round(self.plate.moment_demand / 1000000, 2),
                                                                  ...
                                                                  self.load.axial_force,
                                                                  round(a.tension_capacity / 1000, 2), a.IR),
                ...
                t1 = (DISP_MIN_WELD_SIZE,
                      cl_10_5_2_3_min_fillet_weld_size_required(self.weld_connecting_plates, self.weld_size_min),
                      self.weld.size,
                      get_pass_fail(self.weld_size_min, self.weld.size, relation="leq"))
                self.report_check.append(t1)
                t1 = (DISP_MAX_WELD_SIZE, cl_10_5_3_1_max_weld_size(self.weld_connecting_plates, self.weld_size_max),
                      self.weld.size,
...
if __name__=='__main__':
    finpl = FinPlateConnection()
    finpl.set_input_values()
